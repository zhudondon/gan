...
class VagueToHD:
    def __init__(self):
        ...
        """
            这里是缩小了2**4 大小，结果 看下 辨别器的大小 辨别器 特征提取 缩小了16倍，4次缩小尺寸，拿到特征
        """
        ...
        # 生成假图，假图的特征提取 896 * 896 * 3 ，只要卷积层提取特征，不要 top的连接层，限制 输入大小
    ...
    def build_vgg(self):
        # 建立VGG模型，只使用第9层的特征
        #  可以通过设置 include_top=False 来去掉顶层的全连接层，只保留卷积层，卷积层可以接受任何大小的输入。
    ...
    def train(self, epochs=60000, batch_size=1, sample_interval=60, init_epoch=0):
        ...
        for epoch in range(init_epoch, epochs):
            scheduler([self.combined, self.discriminator], epoch)
            ...
            # 成 224 * 224 * 3
            imgs_hr, imgs_lr, imgs_hr_224 = self.data_loader.load_data_triple(batch_size=batch_size,
            ...
            if epoch > 0 and epoch % sample_interval == 0:
                # 保存图片和权重
                test_input = np.array(
                    [DataLoader.im_read_real(test_image, size=(self.lr_height, self.lr_width))]) / 127.5 - 1.
